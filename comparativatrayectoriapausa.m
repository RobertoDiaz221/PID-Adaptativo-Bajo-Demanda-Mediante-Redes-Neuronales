% Comprehensive robustness evaluation of the proposed adaptive control system
% against baseline strategies on an omnidirectional YouBot.
%
...
%
% Scenario: Rose trajectory tracking subjected to a hard dynamic pause 
% (reference hold and dynamic block) to evaluate anti-windup capabilities, 
% recovery stability, and overall system robustness.
%
...
for k = 2:N
    ...
    % Dynamic Block during hold
    if k >= idx_start && k < idx_end
        state_PID(:,k) = state_PID(:,k-1); % Blocked
    else
...
e_thr_freeze = 0.03;   % Freeze if |e| < 0.03
e_thr_unfz   = 0.05;   % Unfreeze if |e| > 0.05
...
for k = 2:N
    ...
    % Conditional Integral accumulation (avoids static windup)
    if k >= 3
...
title(tl1,'XY Trajectory Tracking (Rose Curve with Hard Pause)');
...
disp('==== COMPARATIVE METRICS (Rose Trajectory + Hard Pause) ====');
disp(metricsTbl);
