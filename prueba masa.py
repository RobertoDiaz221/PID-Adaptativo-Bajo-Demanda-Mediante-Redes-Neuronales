# -*- coding: utf-8 -*-
"""
ZMQ + EKF-SNPID Controller with Hysteresis Freeze and Hamiltonian Prediction.
Includes dynamic payload switching during simulation and gain unfreezing 
triggered by absolute/relative Hamiltonian discrepancies.
"""
...
ML_NEW        = 20.0              # Switched load mass [kg]
...
def ref_original(t, mode):
    """
    Returns (xd, yd, thetad) using original base formulas.
    Modes: "sinusoidal" | "rosa"
    """
    if mode == "sinusoidal":
        xd  = 0.1 * t
...
def main():
    ...
    # Base parameters for Hamiltonian
    ...
    try:
        while True:
            ...
            # --- Hamiltonian Discrepancy Trigger ---
    ...
    ax.set_xlabel('t [s]'); ax.set_ylabel('Control Action'); ax.legend(loc='best'); mark_switch(ax); mark_h_events(ax)
    ...
    ax.legend(['Kp_x','Kd_x','Ki_x']); ax.set_title('X-Axis Gains'); mark_switch(ax); mark_h_events(ax)
    ax=plt.subplot(3,1,2); ax.grid(True)
    if Wy_log.shape[1]>0: ax.plot(t_log, Wy_log[0], t_log, Wy_log[1], t_log, Wy_log[2])
    ax.legend(['Kp_y','Kd_y','Ki_y']); ax.set_title('Y-Axis Gains'); mark_switch(ax); mark_h_events(ax)
    ax=plt.subplot(3,1,3); ax.grid(True)
    if Wt_log.shape[1]>0: ax.plot(t_log, Wt_log[0], t_log, Wt_log[1], t_log, Wt_log[2])
    ax.legend([r'Kp_$\theta$', r'Kd_$\theta$', r'Ki_$\theta$']); ax.set_title(r'$\theta$ Gains'); ax.set_xlabel('t [s]'); mark_switch(ax); mark_h_events(ax)

    if w_log.shape[1]>0:
        plt.figure("Wheel Velocities", figsize=(9,4))
        ax = plt.gca(); ax.grid(True)
        for i in range(4): ax.plot(t_log, w_log[i], label=f'v_{i+1}')
        ax.set_xlabel('t [s]'); ax.set_ylabel('Velocity [rad/s]'); ax.legend(loc='best'); mark_switch(ax); mark_h_events(ax)
    ...
    if H_pred is not None and np.isfinite(H_pred).any():
        ...
        ax.set_title("Absolute Hamiltonian Error"); mark_switch(ax); mark_h_events(ax)
        plt.tight_layout()
